 ...
 * <ul>
 * 		<li><strong> name : String</strong> - A name that is used to identify the MP3Loader instance. This name can be fed to the <code>find()</code> method or Log.outd at any time. Each loader's name should be unique. If you don't define one, a unique name will be created automatically, like "loader21".</li>
 ...
 * @example Example AS3 code:<listing version="3.0">
 ...
 function progressHandler(event:LoaderEvent):void {
 		Log.out("progress: " + event.target.progress);
 }
 
 function completeHandler(event:LoaderEvent):void {
 		Log.out(event.target + " is complete!");
 }
 
 function errorHandler(event:LoaderEvent):void {
 		Log.out("error occured with " + event.target + ": " + event.text);
 }
 </listing>
 * 
