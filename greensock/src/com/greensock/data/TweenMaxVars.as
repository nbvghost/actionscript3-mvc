 ...
 * 		<li> It will Log.out() a warning if you forgot to activate a particular plugin. For example, if you define an autoAlpha value in a TweenMaxVars instance but you didn't activate() the plugin, you'll see a Log.out() output when you test/compile the file (an Error isn't thrown because in some very rare circumstances it can be perfectly legitimate to avoid activating the plugin)</li>
 *  </ol>
 * 
	...
	public class TweenMaxVars {
		...
		public function TweenMaxVars(vars:Object=null) {
			...
			if (TweenMax.version < 11.4) {
				Log.out("WARNING: it is suggested that you update to at least version 11.4 of TweenMax in order for TweenMaxVars to work properly. http://www.greensock.com/tweenmax/"); 
			}
		}
		...
		protected function _set(property:String, value:*, requirePlugin:Boolean=false):TweenMaxVars {
			...
			if (requirePlugin && !(property in TweenLite.plugins)) {
				Log.out("WARNING: you must activate() the " + property + " plugin in order for the feature to work in TweenMax. See http://www.greensock.com/tweenlite/#plugins for details.");
			}
			return this;
		}
