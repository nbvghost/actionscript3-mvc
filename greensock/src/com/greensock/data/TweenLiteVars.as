 ...
 * 		<li> It will Log.out() a warning if you forgot to activate a particular plugin. For example, if you define an autoAlpha value in a TweenLiteVars instance but you didn't activate() the plugin, you'll see a Log.out() output when you test/compile the file (an Error isn't thrown because in some very rare circumstances it can be perfectly legitimate to avoid activating the plugin)</li>
 *  </ol>
 * 
	...
	public class TweenLiteVars {
		...
		public function TweenLiteVars(vars:Object=null) {
			...
			if (TweenLite.version < 11.4) {
				Log.out("WARNING: it is suggested that you update to at least version 11.4 of TweenLite in order for TweenLiteVars to work properly. http://www.greensock.com/tweenlite/"); 
			}
		}
		...
		protected function _set(property:String, value:*, requirePlugin:Boolean=false):TweenLiteVars {
			...
			if (requirePlugin && !(property in TweenLite.plugins)) {
				Log.out("WARNING: you must activate() the " + property + " plugin in order for the feature to work in TweenLite. See http://www.greensock.com/tweenlite/#plugins for details.");
			}
			return this;
		}
...
//---- PLUGIN REQUIRED -------------------------------------------------------------------------------------------
		 ...
		 * simply call reverse() on the timeline to watch the particles reLog.out their steps right back to the beginning. 
		 ...
		 * reLog.out their steps right back to the beginning. Here are the parameters you can define (note that 
