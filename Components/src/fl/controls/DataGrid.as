...
package fl.controls {
     ...
     * </ul>
	 *
     * @includeExample examples/DataGridExample.as -noswf
     *
	...
	public class DataGrid extends SelectableList implements IFocusManagerComponent {
		...
		protected function handleHeaderResizeDown(event:MouseEvent):void {
			...
			myForm.addEventListener(MouseEvent.MOUSE_MOVE, handleHeaderResizeMove, false, 0, true);
			myForm.addEventListener(MouseEvent.MOUSE_UP, handleHeaderResizeUp, false, 0, true);
		}
		...
		protected function handleHeaderResizeUp(event:MouseEvent):void {
			...
			showColumnStretchCursor(false);
			var myForm:DisplayObjectContainer = focusManager.form;
			myForm.removeEventListener(MouseEvent.MOUSE_MOVE, handleHeaderResizeMove, false);
			myForm.removeEventListener(MouseEvent.MOUSE_UP, handleHeaderResizeUp, false);
		}
		...
		protected function showColumnStretchCursor(show:Boolean = true):void {
			...
			if(show) {
				Mouse.hide();
				myForm.addChild(columnStretchCursor);
				myForm.addEventListener(MouseEvent.MOUSE_MOVE, positionColumnStretchCursor, false, 0, true);
				var pt:Point = myForm.globalToLocal(new Point(stage.mouseX, stage.mouseY));
				columnStretchCursor.x = pt.x;
				columnStretchCursor.y = pt.y
			} else {
				myForm.removeEventListener(MouseEvent.MOUSE_MOVE, positionColumnStretchCursor, false);
				if(myForm.contains(columnStretchCursor)) {
					myForm.removeChild(columnStretchCursor);
				}
				Mouse.show();
			}
		}
		...
		protected function positionColumnStretchCursor(event:MouseEvent):void {
			var myForm:DisplayObjectContainer = focusManager.form;
			var pt:Point = myForm.globalToLocal(new Point(event.stageX, event.stageY));
			columnStretchCursor.x = pt.x;
			columnStretchCursor.y = pt.y;
		}
