	...
	public class Button extends LabelButton implements IFocusManagerComponent {
		...
		override public function drawFocus(focused:Boolean):void {
			...
			if (focused) {
				...
				var focusPadding:Object = getStyleValue("focusRectPadding");
				focusPadding = (focusPadding == null) ? 2 : focusPadding;
				focusPadding += emphasizedPadding;
				
				uiFocusRect.x = -Number(focusPadding);
				uiFocusRect.y = -Number(focusPadding);
				uiFocusRect.width = width + (Number(focusPadding)*2);
				uiFocusRect.height = height + (Number(focusPadding)*2);
			}
		}
