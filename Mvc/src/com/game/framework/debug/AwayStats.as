package com.game.framework.debug {
	...
	public class AwayStats extends Sprite {
		private var _views:Vector.<DisplayObject>;
		 ...
		 * can also click the average frame rate Console.out-out to reset just that one
		 * value.
		 *
		 ...
		 * respectively.
		 */
		public function AwayStats() {
			super();
			//view3d:DisplayObject = null, minimized:Boolean = false, transparent:Boolean = false, meanDataLength:uint = 0, enableClickToReset:Boolean = true, enableModifyFrameRate:Boolean = true
			_minimized = false;
			_transparent = false;
			_enable_reset = true;
			_enable_mod_fr = true;
			...
			_views = new Vector.<DisplayObject>();
			//if (view3d)
			//_views.push(view3d);
			...
			if (_INSTANCE)
				Console.out('Creating several statistics windows in one project. Is this intentional?');
			_INSTANCE = this;
		 ...
		 */
		public function registerView(view3d:DisplayObject):void {
		 ...
		 */
		public function unregisterView(view3d:DisplayObject):void {
			if (view3d) {
